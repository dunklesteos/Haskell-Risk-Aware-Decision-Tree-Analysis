...
data RiskTree=Leaf Double | DecisionNode RiskTree RiskTree
                          ...
                          deriving (Show, Read, Eq, Ord)--I saw Ord from lab section and added :)
...
main = do
    ...
    putStrLn "--- Haskell Risk Tree Core Engine Active ---"
    mainLoop
...
mainLoop = do
    ...
    lambdaStr <- getLine
    treeStr <- getLine
    let lmb = read lambdaStr :: Double
        t   = read treeStr :: RiskTree
        cost = riskcost t lmb
        height = risktreeheight t
        (dCount, cCount) = nodetype t
    putStrLn "[RESULT_START]"
    putStrLn $ "Cost:" ++ show cost
    putStrLn $ "Height:" ++ show height
    putStrLn $ "Decisions:" ++ show dCount
    putStrLn $ "Chances:" ++ show cCount
    putStrLn "[RESULT_END]"
    mainLoop